...
func TestPaths(t *testing.T) {
	var paths = &Paths{}
	paths.Add("foo.bar")
	paths.Add("foo.bar.123")
	paths.Add("foo.bar1")
	assert.True(t, paths.Has("foo.bar"))
	assert.True(t, paths.Has("foo.bar.123"))
	assert.True(t, paths.Has("foo.bar1"))
	assert.False(t, paths.Has("lee"))
	assert.False(t, paths.Has("1.2.3.4.5.6"))
	assert.False(t, paths.Has(""))
}

func BenchmarkPaths_Has(b *testing.B) {
	var paths = &Paths{}
	paths.Add("foo.fooddddasdfasdfasdfasdfasdf")
	//var pathss = strings.Split("fooddddasdfasdfasdfasdfasdf.fooddddasdfasdfasdfasdfasdf", ".")
	for i := 0; i < b.N; i++ {
		for j := 0; j < 10; j++ {
			if paths.root.HasName(pp[j][0]) {
			}
		}
...
var t2 = []string{"1234567890123456789", "1234567890123456789"}
var t1 = []string{"1234567890123456789", "1234567890123456789"}

func BenchmarkSametree(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for j := 0; j < 10; j++ {
			for k := j; k < 10; k++ {
				sameTree(pp[k], t1)
