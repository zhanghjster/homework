...
func (p *Parser) EachKey(data []byte, cb func(int, []byte, ValueType, error), paths ...[]string) int {
	var path, curPath = p.paths.root, p.paths.root
	var curName string
	...
	for i < ln {
		...
		case '"':
			...
			if data[i] == ':' {
				...
				// 如果是叶子， 读全部
				// 否则如果有孩子。如果不是 { [ 读全部
				if next, ok := curPath.child[pathsBuf[level-1]]; ok {
					if len(next.child) == 0 && nc != '{' && nc != '[' {
						i++
						p.matched++
...
func (p *Paths) Has(path string) bool {
	return p.root.Has(strings.Split(path, "."))
}

func (p *Paths) Add(path string) {
	if p.root.Add(strings.Split(path, ".")) {
		p.n++
	}
}
...
func (n *PathNode) Add(path []string) bool {
	if len(path) == 0 {
		return true
	}
	...
	if _, ok := n.child[p0]; !ok {
		n.child[p0] = &PathNode{parent: n}
	} else if len(path) == 1 { // reach leaf
		return false
	}

	return n.child[p0].Add(path[1:])
}
