...
1. 安装
   ...
   ```
   pip install virtural-env —upgrade 
   virtualenv —sytem-site-packages ~/tensorflow
...
2. 计算模型，计算图
   ...
   计算图是它的基本计算模型，每个点是个运算，每条边代表计算之间的依赖关系
...
3. 数据模型，张量
   ...
   可以理解为一个多维数组，但张量并未直接保存数据，而是得到这些数字的计算过程，一个张量的结构包含，名称、维度、类型。张量和计算图中的节点是对应的，

   名称（name），用“node:src_output"形式，node是节点的名称，src_output表示当前张量来自节点的第几个输出
